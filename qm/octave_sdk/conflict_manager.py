...
from qm.octave_sdk.connectivity.connectivity import Connectivity, OctaveLOSource, SynthOutputDeviceInfo
from qm.octave_sdk.conflict_exceptions import (
    ConflictException,
...
def _is_synth_output_using_main_source_external(
    synth_out_port: api_pb2.SynthRFOutput.OutputPort, synth_state: api_pb2.SynthUpdate
) -> bool:
    using, main_sw = _is_synth_output_using_main_source(synth_out_port, synth_state)

    return using and main_sw == api_pb2.SynthUpdate.MainSource.MAIN_SOURCE_EXTERNAL


def _is_synth_output_using_internal(
    synth_out_port: api_pb2.SynthRFOutput.OutputPort, synth_state: api_pb2.SynthUpdate
) -> bool:
    using, main_sw = _is_synth_output_using_main_source(synth_out_port, synth_state)

    return using and main_sw == api_pb2.SynthUpdate.MainSource.MAIN_SOURCE_SYNTHESIZER


def _is_synth_output_using_main_source(
    synth_out_port: api_pb2.SynthRFOutput.OutputPort, synth_state: api_pb2.SynthUpdate
) -> Tuple[bool, api_pb2.SynthUpdate.MainSource.ValueType]:  # type: ignore[name-defined]
    other_port_using_main_source: bool = False
    if synth_out_port == api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_SECONDARY:
        if (
            synth_state.secondary_output != api_pb2.SynthUpdate.SecondaryOutput.SECONDARY_OUTPUT_OFF
            and synth_state.secondary_output != api_pb2.SynthUpdate.SecondaryOutput.SECONDARY_OUTPUT_AUXILARY
        ):
            other_port_using_main_source = True
    elif synth_out_port == api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_MAIN:
        if synth_state.main_output != api_pb2.SynthUpdate.MainOutput.MAIN_OUTPUT_OFF:
            other_port_using_main_source = True
...
class Conflict:
    def __init__(self) -> None:
        ...
        self.conflict_up_or_down: Optional[SynthOutputDeviceInfo] = None

    def is_must_raise_error(self) -> bool:
        return False

    def get_is_conflict(self) -> bool:
        return self.is_conflict

    def get_exception_type(self) -> type:
...
class InternalShareConflict(Conflict):
    def __init__(
        ...
        synth_index: int,
        other_port_using_internal: bool,
        synth_out_other: api_pb2.SynthRFOutput.OutputPort,
    ) -> None:
        super().__init__()
...
class InternalExternalOverrideConflict(Conflict):
    def __init__(
        self,
        connectivity: Connectivity,
        synth_state: api_pb2.SynthUpdate,
        synth_index: int,
        main_source_next: api_pb2.SynthUpdate.MainSource,
        ...
        if (
            ...
            and synth_state.main_source != api_pb2.SynthUpdate.MainSource.MAIN_SOURCE_UNSPECIFIED
            and main_source_next != api_pb2.SynthUpdate.MainSource.MAIN_SOURCE_UNSPECIFIED
...
class LoopbackInternalShareConflict(Conflict):
    def __init__(self, connectivity: Connectivity, client: OctaveClient, lo_source: OctaveLOSource, synth_index: int):
        super().__init__()
        ...
        if lo_source in connectivity.loopbacks:
            ...
            if _is_synth_output_using_internal(
                api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_MAIN, synth_state_loopback_source
            ):
                synth_out_other_loopback = api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_MAIN
            if _is_synth_output_using_internal(
                api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_SECONDARY, synth_state_loopback_source
            ):
                synth_out_other_loopback = api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_SECONDARY
        ...
        if lo_source == OctaveLOSource.Internal:
            ...
            if lo_loopback is not None:
                ...
                if _is_synth_output_using_main_source_external(
                    api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_MAIN, synth_state_loopback_sink
                ):
                    synth_out_other_loopback = api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_MAIN
                if _is_synth_output_using_main_source_external(
                    api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_SECONDARY, synth_state_loopback_sink
                ):
                    synth_out_other_loopback = api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_SECONDARY
...
class SynthConflicts:
    synth_state: Optional[api_pb2.SynthUpdate] = None

    def __init__(
        self,
        synth_index: int,
        synth_out_to_be_used: api_pb2.SynthRFOutput.OutputPort,
        lo_source: OctaveLOSource,
        connectivity: Connectivity,
        client: OctaveClient,
        ...
        main_source_next = api_pb2.SynthUpdate.MainSource.MAIN_SOURCE_UNSPECIFIED
        if lo_source == OctaveLOSource.Off:
            pass
        elif lo_source == OctaveLOSource.Internal:
            main_source_next = api_pb2.SynthUpdate.MainSource.MAIN_SOURCE_SYNTHESIZER
        elif synth_out_to_be_used == api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_MAIN:
            main_source_next = api_pb2.SynthUpdate.MainSource.MAIN_SOURCE_EXTERNAL

        # check if using internal in the other port
        if synth_out_to_be_used == api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_MAIN:
            synth_out_other = api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_SECONDARY
        else:
            synth_out_other = api_pb2.SynthRFOutput.OutputPort.OUTPUT_PORT_MAIN
    ...
    def print_error(self) -> None:
        ...
        for conflict in self._conflicts:
            if conflict.get_is_conflict():
                assert conflict.conflict_up_or_down is not None
                print_string = (

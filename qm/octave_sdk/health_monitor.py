...
from qm.octave_sdk.health_client import HealthClient
from qm.octave_sdk.connectivity.connectivity import ModulesSlotsFromIdentity
from qm.octave_sdk._octave_client import MonitorData, OctaveClient, ExploreResult, MonitorResult
...
class HealthMonitor:
    ...
    def __init__(
        ...
        client: OctaveClient,
        name: Optional[str],
        reset_connectivity: Callable[[ExploreResult], None],
        slots_list: ModulesSlotsFromIdentity,
    ...
    def _health_update(self, explore_result: ExploreResult, monitor_result: MonitorResult) -> None:
        ...
        if not errors_found:
            logger.debug(f'Octave "{self._octave_name}" Health check passed, current temperature {int(max_temp)}')

    def _translate_error_to_string(
        self, module_name: api_pb2.OctaveModule, module_index: int, monitor_data: MonitorData
    ) -> None:
        list_of_errors = "".join(
            f' "{api_pb2.MonitorResponse.OctaveError.Name(err.type)}"' for err in monitor_data.errors
        )
    ...
    def _is_module_in_identity(self, name: api_pb2.OctaveModule, slot_index: int) -> bool:
        slot_list = []

        if name == api_pb2.OctaveModule.OCTAVE_MODULE_RF_UPCONVERTER:
            slot_list = self._slot_list.rf_out_list
        elif name == api_pb2.OctaveModule.OCTAVE_MODULE_RF_DOWNCONVERTER:
            slot_list = self._slot_list.rf_in_list
        elif name == api_pb2.OctaveModule.OCTAVE_MODULE_IF_DOWNCONVERTER:
            slot_list = self._slot_list.if_list
        elif name == api_pb2.OctaveModule.OCTAVE_MODULE_SYNTHESIZER:
            slot_list = self._slot_list.synth_list
        elif name == api_pb2.OctaveModule.OCTAVE_MODULE_MOTHERBOARD or name == api_pb2.OctaveModule.OCTAVE_MODULE_SOM:
            slot_list = [1]
